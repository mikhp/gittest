...
x, y = int(screenWidth / 4), int(screenHeight /
                                 4)  # 初始运行窗口屏幕坐标(x, y),设置成在左上角显示
...
code_widget = tix.Entry(code_frame,
                        textvariable=input_code_var,
                        borderwidth=1,
                        justify=CENTER)
...
startdate_widget = tix.Entry(input_date_frame,
                             textvariable=input_startdate_var,
                             borderwidth=1,
                             justify=CENTER)
input_startdate_get = input_startdate_var.set(
    input_startdate_var.get())  # 获取输入的新值
...
enddate_widget = tix.Entry(input_date_frame,
...
def go():
    ...
    stock_data = pro.daily(ts_code=code_name,
    ...
    data.set_index(
        'Date', inplace=True)  # 设置date列为索引，覆盖原来索引,这个时候索引还是 object 类型，就是字符串类型。
...
# 在主框架下创建查询按钮子框架
search_frame = tix.Frame(main_frame,
                         borderwidth=1,
                         bg='#353535',
                         relief=tix.SUNKEN)
search_frame.pack()
